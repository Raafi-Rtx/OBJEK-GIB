...
HideButton.Parent = DemonologyPRO
HideButton.Size = UDim2.fromOffset(32, 32)
HideButton.Position = UDim2.fromScale(0.03, 0.5)
HideButton.Text = "≡"
HideButton.Font = Enum.Font.GothamBold
HideButton.TextSize = 16
HideButton.BackgroundColor3 = Color3.fromRGB(0, 170, 0)
HideButton.TextColor3 = Color3.new(1, 1, 1)
HideButton.Active = true
HideButton.ZIndex = 10000
...
Temperature.Text = "Temperature: 0C"
...
GhostWriting.Text = "Ghost Writing: No"
...
LaserProjector.Text = "Laser Projector: No"
...
Ghost.Text = "Ghost: Male | Age 69 | Favorite room: ..."
...
AutoSpiritBox.Text = "Auto spirit box"
...
TakeAPhoto.Text = "Take a photo ghost"
...
CheckSpeed.Text = "Check speed: 1s"
...
ToggleLights.Text = "Toggle all lights"
...
ModeText.Text = "Difficulty: Unknow"
...
Take3StarsPhoto.Text = "Take 3 stars photo"
...
EvidenceEsp.Text = "Handprints, Orb esp"
...
NoclipBtn.Parent = Buttons2
NoclipBtn.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
NoclipBtn.BorderColor3 = Color3.fromRGB(0, 0, 0)
NoclipBtn.BorderSizePixel = 0
NoclipBtn.Position = UDim2.new(0.370035648, 0, 0.759845376, 0)
NoclipBtn.Size = UDim2.new(0.230000004, 0, 0.200000003, 0)
...
AntiLagBtn.Parent = Buttons2
AntiLagBtn.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
AntiLagBtn.BorderColor3 = Color3.fromRGB(0, 0, 0)
AntiLagBtn.BorderSizePixel = 0
AntiLagBtn.Position = UDim2.new(0.699133813, 0, 0.759845376, 0)
AntiLagBtn.Size = UDim2.new(0.230000004, 0, 0.200000003, 0)
AntiLagBtn.Font = Enum.Font.SourceSans
AntiLagBtn.Text = "antilag"
AntiLagBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
AntiLagBtn.TextScaled = true
AntiLagBtn.TextSize = 14.000
		...
		for _, plr in pairs(game:GetService("Players"):GetPlayers()) do
			if plr.Character then
				...
				if HumanoidRootPart then
					local Esp1 = HumanoidRootPart:FindFirstChild("ScriptPlrBil")
					local Esp2 = HumanoidRootPart:FindFirstChild("ScriptPlrEsp")
					...
					if not Esp1 and not Esp2 then
						...
						BillboardGui.Name = "ScriptPlrBil"
	...
	if EvidenceEspToggle then
		for _, obj in ipairs(workspace.Handprints:GetDescendants()) do
			if obj:IsA("BasePart") then
				...
				BillboardGui.Name = "ScriptHandprintsBil"
				BillboardGui.Parent = game:GetService("CoreGui")
				BillboardGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
				BillboardGui.Active = true
				BillboardGui.AlwaysOnTop = true
		...
		if GhostOrbPart then
			...
			BillboardGui.Name = "ScriptOrbBil"
			BillboardGui.Parent = game:GetService("CoreGui")
			BillboardGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
			BillboardGui.Active = true
			BillboardGui.AlwaysOnTop = true
...
local function CheckHandprints()
	-- Cek di workspace.Handprints
	local handprintsFolder = workspace:FindFirstChild("Handprints")
	if handprintsFolder then
		for _, obj in ipairs(handprintsFolder:GetChildren()) do
			if obj:IsA("BasePart") and validNames[obj.Name] then
				return true
			end
		end
	end

	-- Cek di workspace.DisturbedSaltLine
	local saltFolder = workspace:FindFirstChild("DisturbedSaltLine")
	if saltFolder then
		for _, obj in ipairs(saltFolder:GetChildren()) do
			if obj:IsA("BasePart") and validNames[obj.Name] then
				return true
			end
		end
...
local function CheckWither()
	...
	for _, obj in ipairs(workspace.Items:GetDescendants()) do
		if obj:IsA("BasePart") and obj.Name == "Flower_Petals" then
	...
	if tick() - OldTick > CheckSpeedNum then --delay 0.2s
		...
		if GhostGender and GhostAge and GhostFavRoom then
			if GhostHunting == true then
				...
				Ghost.Text = "Ghost: "..GhostGender.." | Age: "..tostring(GhostAge).." | Favorite room: "..GhostFavRoom
				Ghost.TextColor3 = Color3.fromRGB(255, 255, 255)
			end
		else
			if GhostHunting == true then
				Ghost.Text = "Ghost: Hunting"
				Ghost.TextColor3 = Color3.fromRGB(255, 0, 0)
			else
				Ghost.Text = "Ghost: Chilling"
				Ghost.TextColor3 = Color3.fromRGB(255, 255, 255)
			end
		end

		if GhostCurrentRoom then
			CurrentRoom.Text = "Current room: " .. GhostCurrentRoom
		end
		...
		if Temp and TempRoom then
			if Temp < LowestTemp then
				...
				Temperature.Text = string.format("Temperature: %.1f°C", LowestTemp) .. " (" .. tostring(LowestTempRoom.Name) .. ")"
		...
		if EMFLevelCheck > HighestEMFLevel then
			...
			if EMFLevelCheck == 5 then
				EMF.Text = "EMF level: " .. tostring(HighestEMFLevel)
...
PlaceStuff.MouseButton1Click:Connect(function()
	-- BAGIAN PENGECEKAN KEAMANAN DAN DEBOUNCE SUDAH DIHAPUS
	-- Script langsung masuk ke eksekusi item
	...
	task.wait(0.5) -- Mulai drop ronde 1
	...
	task.wait(0.5) -- Mulai drop ronde 2
	...
	-- Tidak ada TpOutside dan tidak ada db = false
end)
...
CheckSpeed.MouseButton1Click:Connect(function()
	...
	CheckSpeed.Text = "Check speed: " .. tostring(Numbers[index]) .. "s"
end)
...
-- Ganti atau tambah folder item sesuai struktur game
local ItemFolders = {
		...
		for _, folder in ipairs(ItemFolders) do
			if folder then
				for _, v in ipairs(folder:GetChildren()) do
					if v:IsA("Model") and v:GetAttribute("ItemName") then
						-- BillboardGui (ukuran kecil)
						local BillboardGui = Instance.new("BillboardGui")
						BillboardGui.Name = "ScriptItemBil"
						BillboardGui.Parent = v
						BillboardGui.Adornee = v
						BillboardGui.AlwaysOnTop = true
						BillboardGui.LightInfluence = 0
						BillboardGui.Size = UDim2.new(0, 60, 0, 20)
...
--========================================
-- AUTO LOCK WALKSPEED (PROPERTY BASED)
-- LocalScript
--========================================
...
-- SPEED CONFIG
-- ================================
local SpeedNumbers = {0, 20, 25, 30, 35, 40}
local SpeedIndex = 2 -- default 20

local TARGET_SPEED = SpeedNumbers[SpeedIndex]
...
-- APPLY SPEED LOCK
-- ================================
local function applySpeedLock(char)
	...
	-- Kunci WalkSpeed (efisien & ramah)
	connection = humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
		if loopEnabled and humanoid.WalkSpeed ~= TARGET_SPEED then
			humanoid.WalkSpeed = TARGET_SPEED
		end
	end)
end
...
Speedy.MouseButton1Click:Connect(function()
	...
	local newSpeed = SpeedNumbers[SpeedIndex]
	...
	Speedy.Text = "Speed: " .. tostring(TARGET_SPEED)
end)

Infyield.MouseButton1Click:Connect(function()
    -- Baris loadstring Infinite Yield dihapus
    -- Diganti dengan perintah teleport ke hantu
    TpToGhost()
end)
...
FullBright.MouseButton1Click:Connect(function()
    if FullBrightToggle == false then
        ...
        Lighting.OutdoorAmbient = OldLightingList.OutdoorAmbient
        Lighting.Brightness = OldLightingList.Brightness
        Lighting.ClockTime = OldLightingList.ClockTime
        Lighting.GlobalShadows = OldLightingList.GlobalShadows
        Lighting.FogEnd = OldLightingList.FogEnd
    end
end)
...
local function AlreadyTakenCheck(Obj)
	...
	if Obj:GetAttribute("ScriptAlreadyTakenPhoto") == true then
		AlreadyTaken = true
	end
	return AlreadyTaken
end
		...
		for _, v in pairs(workspace.Items:GetChildren()) do
			if v:GetAttribute("DisplayName") == "Burnt Cross" then
				if not AlreadyTakenCheck(v) then
					v:SetAttribute("ScriptAlreadyTakenPhoto", true)
					Item = v
					ItemType = "BurntCross"
...
AntiLagBtn.MouseButton1Click:Connect(function()
	getgenv().AntiLagEnabled = not getgenv().AntiLagEnabled
	...
	if getgenv().AntiLagEnabled then
		...
		AntiLagBtn.Text = "AntiLag: OFF"
		warn("AntiLag NONAKTIF (perlu rejoin untuk reset)")
	end
end)